...
public class ViewContractDetailServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            ArrayList<String> img = RoomDAO.getRoomImage(contract.getRoID());
            request.setAttribute("imgList", img);
            
            request.setAttribute("owner", OwnerDAO.getOwnerByOID(contract.getOID()));
            request.setAttribute("user", UserDAO.getUserByUID(contract.getUID()));
//            out.print(contract.toString());
