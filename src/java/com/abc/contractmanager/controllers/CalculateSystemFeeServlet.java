...
import static com.abc.contractmanager.utils.Utilities.getDate;
import static com.abc.contractmanager.utils.Utilities.getDay;
import static com.abc.contractmanager.utils.Utilities.getMonth;
import static com.abc.contractmanager.utils.Utilities.getYear;
 ...
 * @author mical
 */
public class CalculateSystemFeeServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            request.setAttribute("intRentalFee", rentalFee);
            boolean brk = false;
            int userID = 0;
...
//            request.getRequestDispatcher("CreateContract.jsp").forward(request, response);
            if (!brk) {
                BigDecimal systemFee = Utilities.calculateSystemFee(Utilities.countMonth(from, to));
                request.setAttribute("systemFee", systemFee.floatValue());                
                if (owner.getCredit().compareTo(systemFee) <0){
                    request.setAttribute("creditNoti", "Your credit is not enough to create this contract!");
                }else{
                    request.setAttribute("cstatus", 1);
                }
            } else {
                request.setAttribute("noti", "Calculate failed!");
            }
            request.getRequestDispatcher("CreateContract.jsp").forward(request, response);
        }
    }
